...
import css from './OurMissionPage.module.css';
import OurMission from './OurMission';
import OurMissionIT from './OurMissionIT';
import useLocale from '../../hooks/useLocale';

const OurMissionPageComponent = props => {
  ...
  const schemaTitle = intl.formatMessage({ id: 'OurMissionPage.schemaTitle' }, { siteTitle });
  const schema = {
  ...
  const NewComponent = useLocale(OurMissionIT, OurMission);
  ...
  return (
    <Page title={schemaTitle} scrollingDisabled={scrollingDisabled} schema={schema}>
      <LayoutSideNavigation>
        <LayoutWrapperTopbar>
          <TopbarContainer currentPage="OurMissionPage" />
        </LayoutWrapperTopbar>
        <LayoutWrapperSideNav tabs={tabs} type="generic" />
        <LayoutWrapperMain>
          <div className={css.content}>
            <h1 className={css.heading}>
              <FormattedMessage id="OurMissionPage.heading" />
            </h1>
            {/* <OurMission /> */}
            {NewComponent}
          </div>
...
OurMissionPageComponent.propTypes = {
