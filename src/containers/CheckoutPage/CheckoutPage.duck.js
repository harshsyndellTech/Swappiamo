...
export const initiateOrder = (orderParams, transactionId, priceValues) => (
  dispatch,
  getState,
  sdk
) => {
  const isPaid = orderParams?.priceFlag;
  dispatch(initiateOrderRequest());
  // console.log('priceValues', priceValues);
  const orderParamsNew =
    orderParams?.category === 'service' ||
    orderParams?.category === 'event' ||
    orderParams?.category === 'vacanze'
      ? {
      ...
      : {
          listingId: orderParams.listingId,
          stockReservationQuantity: orderParams?.quantity,
        };
  ...
  const processAlias = isPaid
    ? orderParams?.category === 'service' || orderParams?.category === 'event'
      ? config.bookingPaidProcessAlias
      : orderParams?.category === 'vacanze'
      ? config.bookingDailyPaidProcessAlias
      : config.productPaidProcessAlias
    : orderParams?.category === 'service' || orderParams?.category === 'event'
    ? config.bookingProcessAlias
    : orderParams?.category === 'vacanze'
    ? config.bookingDailyProcessAlias
    : config.productProcessAlias;
    ...
    : {
        ...
        params: {
          ...
          metadata: {
            ...
            address: orderParams?.address,
            isPaid,
            // locked_credits: priceValues?.totalAmount,
          },
        },
      };
  const queryParams = {
 ...
 */
export const speculateTransaction = (orderParams, transactionId, priceValues) => (
  dispatch,
  getState,
  sdk
) => {
