...
export class CheckoutPageComponent extends Component {
  ...
  loadInitialData() {
    ...
    const isCategoryProduct = pageData?.listing?.attributes?.publicData?.category === 'product';
    if (shouldFetchSpeculatedTransaction || isCategoryProduct) {
      const listingId = pageData.listing.id;
      const transactionId = tx ? tx.id : null;
      const { bookingStart, bookingEnd } = pageData.bookingDates;
      // console.log('pageData', pageData);
      const orderParams = pageData?.bookingData?.orderParams;
      const priceValues = pageData?.bookingData?.priceValues;
      // Fetch speculated transaction for showing price in booking breakdown
      // NOTE: if unit type is line-item/units, quantity needs to be added.
      // The way to pass it to checkout page is through pageData.bookingData
      fetchSpeculatedTransaction(orderParams, transactionId, priceValues);
    }
  ...
  handlePaymentIntent(handlePaymentParams) {
    ...
    const fnRequestPayment = fnParams => {
      ...
      return hasPaymentIntents
        ...
        : onInitiateOrder(fnParams, storedTx.id, priceValues);
    };
  ...
  render() {
    ...
    const timeZone = currentListing.attributes.availabilityPlan
      ...
      : 'Etc/UTC';
    const isVacanzeCategory = currentListing.attributes.publicData.category === 'vacanze';

    const breakdown = tx.id ? (
      <BookingBreakdown
        className={css.bookingBreakdown}
        userRole="customer"
        unitType={isVacanzeCategory ? 'line-item/night' : config.bookingUnitType}
        transaction={tx}
        booking={txBooking}
        dateType={DATE_TYPE_DATETIME}
        timeZone={timeZone}
        priceValues={this.state.pageData.bookingData.priceValues}
        isProductCategory={currentListing.attributes.publicData.category === 'product'}
    ...
    const detailsSubTitle = `${formattedPrice} ${intl.formatMessage({ id: priceLabel })}`;
    ...
    const initalValuesForStripePayment = { name: userName, country: 'IT' };
...
const mapDispatchToProps = dispatch => ({
  dispatch,
  fetchSpeculatedTransaction: (params, transactionId, priceValues) =>
    dispatch(speculateTransaction(params, transactionId, priceValues)),
