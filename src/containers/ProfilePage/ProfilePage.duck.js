...
import { onFetchUser } from '../../util/api';
...
export default function profilePageReducer(state = initialState, action = {}) {
  ...
  switch (type) {
    ...
    case SHOW_USER_REQUEST:
      return { ...state, userShowError: null, userId: payload.userId, userLoading: true };
    case SHOW_USER_SUCCESS:
      return { ...state, user: payload, userLoading: false };
    // return state;
    case SHOW_USER_ERROR:
      return { ...state, userShowError: payload, userLoading: false };
...
export const showUser = userId => (dispatch, getState, sdk) => {
  ...
  if (currentUser) {
    return onFetchUser({ userId })
      .then(response => {
        const data = {
          ...response.data.data,
          profileImage: response?.data?.included ? response?.data?.included[0] : null,
        };
...
export const loadData = params => async (dispatch, getState, sdk) => {
  ...
  return await Promise.all([
    ...
    await dispatch(queryUserListings(userId)),
    await dispatch(queryUserReviews(userId)),
    await dispatch(showUser(userId)),
  ]);
};
