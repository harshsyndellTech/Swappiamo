...
import { onFetchUsers } from '../../util/api';
...
export const UPDATE_SEARCH_LISTINGS = 'app/SearchPage/UPDATE_SEARCH_LISTINGS';
...
export const FETCH_USERS_REQUEST = 'app/SearchPage/FETCH_USERS_REQUEST';
export const FETCH_USERS_SUCCESS = 'app/SearchPage/FETCH_USERS_SUCCESS';
export const FETCH_USERS_ERROR = 'app/SearchPage/FETCH_USERS_ERROR';
// ================ Reducer ================ //
...
const listingPageReducer = (state = initialState, action = {}) => {
  ...
  switch (type) {
    ...
    case SEARCH_LISTINGS_SUCCESS:
      return {
        ...state,
        currentPageResultIds: removeDuplicates([
...
export const fetchUsersForFilter = () => (dispatch, getState, sdk) => {
  dispatch({ type: FETCH_USERS_REQUEST });
  return onFetchUsers()
    .then(response => {
      const users = response;
      const userData = users.map(user => ({
        label: user.attributes.profile.displayName,
        value: user.id.uuid,
        image: user?.profileImage?.image?.attributes?.variants['square-small']?.url,
      }));
...
export const FetchNextPage = (page, search) => (dispatch, getState, sdk) => {
  const { address, origin, ...rest } = search;
  const originMaybe = config.sortSearchByDistance && origin ? { origin } : {};
  dispatch({ type: UPDATE_SEARCH_LISTINGS, payload: page });
  const params = {
    ...rest,
    ...originMaybe,
    page,
