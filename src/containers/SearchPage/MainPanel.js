...
import { categoryOptions, productCategoryOption } from '../../marketplace-custom-config';
...
class MainPanel extends Component {
  ...
  render() {
    ...
    const serviceFilter = ['category', 'serviceCategory', 'price'];
    const productFilter = ['category', 'category', 'price'];
    const subCategory = ['category', 'subCategory', 'price'];
    const defaultFilter = ['category', 'price'];

    // if (category.includes('service')) {
    //   filterConfig = serviceFilter?.map(f => filterConfig.find(g => g.id === f));
    // } else if (category.includes('product')) {
    //   filterConfig = productFilter?.map(f => filterConfig.find(g => g.id === f));
    // } else {
    //   filterConfig = defaultFilter?.map(f => filterConfig.find(g => g.id === f));
    // }

    const serviceCatSelected = category?.includes('service');
    const productCatSelected = category?.includes('product');

    if (productCatSelected || serviceCatSelected) {
      filterConfig = subCategory.map(f => filterConfig.find(g => g.id === f));

      let subCatOptions = [];
      productCatSelected ? (subCatOptions = productCategoryOption) : null;
      serviceCatSelected ? (subCatOptions = [...subCatOptions, ...categoryOptions]) : null;

      const idx = filterConfig.findIndex(e => e?.id == 'subCategory');
      let subCatFilter = filterConfig[idx];
      subCatFilter.config.options = subCatOptions;

      if (idx > 0) {
        filterConfig[idx] = subCatFilter;
      }
    } else {
      filterConfig = defaultFilter?.map(f => filterConfig.find(g => g.id === f));
    }
