...
import css from './HowSwappimoWorksPage.module.css';
import HowSwappimoWorks from './HowSwappimoWorks';
import HowSwappimoWorksIT from './HowSwappimoWorksIT';
import useLocale from '../../hooks/useLocale';

const HowSwappimoWorksPageComponent = props => {
  ...
  const schemaTitle = intl.formatMessage({ id: 'HowSwappimoWorksPage.schemaTitle' }, { siteTitle });
  const schema = {
  ...
  const NewComponent = useLocale(HowSwappimoWorksIT, HowSwappimoWorks);
  ...
  return (
    <Page title={schemaTitle} scrollingDisabled={scrollingDisabled} schema={schema}>
      <LayoutSideNavigation>
        <LayoutWrapperTopbar>
          <TopbarContainer currentPage="HowSwappimoWorksPage" />
        </LayoutWrapperTopbar>
        <LayoutWrapperSideNav type="generic" tabs={tabs} />
        <LayoutWrapperMain>
          <div className={css.content}>
            <h1 className={css.heading}>
              <FormattedMessage id="HowSwappimoWorksPage.heading" />
            </h1>
            {/* <NewComponent /> */}
            {NewComponent}
          </div>
...
HowSwappimoWorksPageComponent.propTypes = {
...
const HowSwappimoWorksPage = compose(
  connect(mapStateToProps),
  injectIntl
)(HowSwappimoWorksPageComponent);

export default HowSwappimoWorksPage;
