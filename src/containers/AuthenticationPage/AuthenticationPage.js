...
export class AuthenticationPageComponent extends Component {
  ...
  render() {
    ...
    console.log({ url: match.url, id: match.params.id });
    ...
    const exactJoinURL = `/join/${match.params.id}`;
    const isJoinLink = currentURL === exactJoinURL;
    ...
    const handleSubmitSignup = values => {
      const { fname, lname, confirmation_password, ...rest } = values;
      const params = {
      ...
      const expectedJoinURL = match.url;
      const exactJoinURL = `/join/${match.params.id}`;

      if (expectedJoinURL === exactJoinURL) {
        return submitInviteSignup(params);
      } else {
    ...
    const handleSubmitConfirm = values => {
      ...
      const invitedByUserId = match?.params?.id;
    ...
    const getDefaultRoutes = () => {
      ...
      const defaultConfirmParam = defaultConfirm ? `&defaultConfirm=${defaultConfirm}/${id}` : '';
...
const mapDispatchToProps = dispatch => ({
  ...
  submitInviteSignup: params => dispatch(submitInviteSignup(params)),
