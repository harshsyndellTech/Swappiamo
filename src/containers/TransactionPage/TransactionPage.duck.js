...
export const ACCEPT_CUSTOMER_REQUEST = 'app/TransactionPage/ACCEPT_CUSTOMER_REQUEST';
export const ACCEPT_CUSTOMER_SUCCESS = 'app/TransactionPage/ACCEPT_CUSTOMER_SUCCESS';
export const ACCEPT_CUSTOMER_ERROR = 'app/TransactionPage/ACCEPT_CUSTOMER_ERROR';

export const PRODUCT_RECEIVED_REQUEST = 'app/TransactionPage/PRODUCT_RECEIVED_REQUEST';
export const PRODUCT_RECEIVED_SUCCESS = 'app/TransactionPage/PRODUCT_RECEIVED_SUCCESS';
export const PRODUCT_RECEIVED_ERROR = 'app/TransactionPage/PRODUCT_RECEIVED_ERROR';

export const DECLINE_CUSTOMER_REQUEST = 'app/TransactionPage/DECLINE_CUSTOMER_REQUEST';
export const DECLINE_CUSTOMER_SUCCESS = 'app/TransactionPage/DECLINE_CUSTOMER_SUCCESS';
export const DECLINE_CUSTOMER_ERROR = 'app/TransactionPage/DECLINE_CUSTOMER_ERROR';
// ================ Reducer ================ //
...
const initialState = {
  ...
  acceptCustomerProgress: false,
  declineCustomerProgress: false,
  acceptCustomerError: null,
  ...
  productReceivedInProgress: false,
...
export default function checkoutPageReducer(state = initialState, action = {}) {
  ...
  switch (type) {
    ...
    case ACCEPT_CUSTOMER_REQUEST:
      return {
        ...state,
        acceptCustomerProgress: true,
        acceptCustomerError: null,
        declineCustomerError: null,
      };
    case ACCEPT_CUSTOMER_SUCCESS:
      return { ...state, acceptCustomerProgress: false };
    case ACCEPT_CUSTOMER_ERROR:
      return { ...state, acceptCustomerProgress: false, acceptCustomerError: payload };
    case PRODUCT_RECEIVED_REQUEST:
      return { ...state, productReceivedInProgress: true, productReceivedError: null };
    case PRODUCT_RECEIVED_SUCCESS:
      return { ...state, productReceivedInProgress: false };
    case PRODUCT_RECEIVED_ERROR:
      return { ...state, productReceivedInProgress: false, productReceivedError: payload };
    case DECLINE_CUSTOMER_REQUEST:
      return {
        ...state,
        declineCustomerProgress: true,
        declineCustomerError: null,
      };
    case DECLINE_CUSTOMER_SUCCESS:
      return { ...state, declineCustomerProgress: false };
    case DECLINE_CUSTOMER_ERROR:
      return { ...state, declineCustomerProgress: false, acceptCustomerError: payload };
...
const acceptCustomerRequest = () => ({ type: ACCEPT_CUSTOMER_REQUEST });
const acceptCustomerSuccess = () => ({ type: ACCEPT_CUSTOMER_SUCCESS });
const acceptCustomerError = e => ({ type: ACCEPT_CUSTOMER_ERROR, error: true, payload: e });

const productReceivedRequest = () => ({ type: PRODUCT_RECEIVED_REQUEST });
const productReceivedSuccess = () => ({ type: PRODUCT_RECEIVED_SUCCESS });
const productReceivedError = e => ({ type: PRODUCT_RECEIVED_ERROR, error: true, payload: e });

const declineCustomerRequest = () => ({ type: DECLINE_CUSTOMER_REQUEST });
const declineCustomerSuccess = () => ({ type: DECLINE_CUSTOMER_SUCCESS });
const declineCustomerError = e => ({ type: DECLINE_CUSTOMER_ERROR, error: true, payload: e });
...
export const acceptSale = id => (dispatch, getState, sdk) => {
  ...
  //  sdk.transactions
  // .transition({ id, transition: TRANSITION_OFFER_ACCEPT, params: {} }, { expand: true })
  return acceptOffer({
    id,
  })
    .then(response => {
...
export const customerAcceptOffer = (id, customer, price) => (dispatch, getState, sdk) => {
  dispatch(acceptCustomerRequest());
  return customerAccept({
    id,
  })
    .then(response => {
      dispatch(addMarketplaceEntities(response));
      dispatch(acceptCustomerSuccess());
      dispatch(fetchCurrentUserNotifications());
      return response;
    })
    .catch(e => {
      dispatch(acceptCustomerError(storableError(e)));
    });
};
export const customerReceivedProduct = id => (dispatch, getState, sdk) => {
  dispatch(productReceivedRequest());
  ...
  return productReceived({
    ...
    .then(response => {
      dispatch(addMarketplaceEntities(response));
      dispatch(productReceivedSuccess());
      dispatch(fetchCurrentUserNotifications());
      return response;
    })
    .catch(e => {
      dispatch(productReceivedError(storableError(e)));
      log.error(e, 'accept-customer-failed', {
        txId: id,
        transition: TRANSITION_PRODUCT_RECEIVED,
      });
      throw e;
    });
};
export const customerCancelOffer = id => (dispatch, getState, sdk) => {
  dispatch(declineCustomerRequest());
  return cancelCustomerOrder({ id })
    .then(response => {
      dispatch(addMarketplaceEntities(response));
      dispatch(declineCustomerSuccess());
      dispatch(fetchCurrentUserNotifications());
      return response;
    })
    .catch(e => {
      dispatch(declineCustomerError(storableError(e)));
    });
};
