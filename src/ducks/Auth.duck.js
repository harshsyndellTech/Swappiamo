...
import { createUserWithIdp, submitInviteSignupToAPI } from '../util/api';
...
export const signup = params => (dispatch, getState, sdk) => {
  ...
  const createUserParams = isEmpty(rest)
    ? { email, password, firstName, lastName, privateData: { phoneNumber } }
    : {
        email,
        password,
        firstName,
...
export const submitInviteSignup = params => (dispatch, getState, sdk) => {
  if (authenticationInProgress(getState())) {
    return Promise.reject(new Error('Login or logout already in progress'));
  }
  dispatch(signupRequest());
  const { email, password, firstName, lastName, id, phoneNumber, ...rest } = params;
  ...
  return submitInviteSignupToAPI(createUserParams)
    .then(() => dispatch(signupSuccess()))
    .then(() => dispatch(login(email, password)))
    .catch(e => {
      dispatch(signupError(storableError(e)));
