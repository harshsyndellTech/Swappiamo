...
import { injectIntl, intlShape, useIntl } from '../../util/reactIntl';
    ...
    <fieldset className={className}>
      <ul className={css.list}>
        {options.map((option, index) => {
          ...
          const label = intl.formatMessage({ id: option.label });

          return (
            <li key={fieldId} className={css.item}>
              <FieldCheckbox id={fieldId} name={name} label={label} value={option.key} />
            </li>
          );
        })}
      </ul>
