...
class CurrencyInputComponent extends Component {
  constructor(props) {
    ...
    try {
      ...
      const final = hasInitialValue
        ? intl.formatNumber(ensureDotSeparator(unformattedValue), currencyConfig)?.split(/(\s+)/)
        : '';
      const formattedValue = hasInitialValue ? `${final[2]} ${final[0]}` : '';
  ...
  updateValues(event) {
    try {
      ...
      const final = !isEmptyString
        ? intl.formatNumber(ensureDotSeparator(unformattedValue), currencyConfig)?.split(/(\s+)/)
        : '';
      const formattedValue = !isEmptyString ? `${final[2]} ${final[0]}` : '';
