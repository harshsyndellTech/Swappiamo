...
const resolveTransitionMessage = (
  ...
  const isProduct = transaction?.attributes?.metadata?.category === 'product';
  const isOwnTransition = transition.by === ownRole;
  const currentTransition = transition.transition;
  const displayName = otherUsersName;
  const acceptOwnMessage = isProduct ? (
    <FormattedMessage id="ActivityFeed.ownTransitionProductAccept" />
  ) : (
  ...
  switch (currentTransition) {
    ...
    case TRANSITION_ACCEPT:
      if (isProduct) {
        return isOwnTransition ? (
          ...
          <FormattedMessage id="ActivityFeed.transitionProductAccept" values={{ displayName }} />
        );
      } else {
        return isOwnTransition ? (
          <FormattedMessage id="ActivityFeed.ownTransitionAccept" />
    ...
    case TRANSITION_DECLINE:
      return isOwnTransition ? (
        <FormattedMessage
          id={
            isProduct
              ? 'ActivityFeed.ownTransitionDeclineProduct'
        ...
        <FormattedMessage
          id={
            isProduct ? 'ActivityFeed.transitionDeclineProduct' : 'ActivityFeed.transitionDecline'
          }
          values={{ displayName }}
        />
      );
    ...
    case TRANSITION_COMPLETE_END:
      ...
      const reviewAsFirstLink = reviewPeriodJustStarted ? (
        <SecondaryButton onClick={onOpenReviewModal} className={css.reviewButton}>
      ...
      return (
        <div>
          <FormattedMessage
            id={
              isProduct
                ? 'ActivityFeed.transitionProductComplete'
                : 'ActivityFeed.transitionComplete'
            }
    ...
    case TRANSITION_REVIEW_1_BY_CUSTOMER:
      if (isOwnTransition) {
        ...
        const reviewAsSecondLink = !(reviewPeriodIsOver || userHasLeftAReview) ? (
          <InlineTextButton onClick={onOpenReviewModal} className={css.secondReview}>
...
const Transition = props => {
  const { transition, transaction, currentUser, intl, onOpenReviewModal, isProduct } = props;
