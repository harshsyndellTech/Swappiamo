...
export let TABS = [DESCRIPTION, LOCATION, ...availabilityMaybe, PRICING, PHOTOS];
...
const tabCompleted = (tab, listing) => {
  ...
  switch (tab) {
    ...
    case PRICING:
      return !!(publicData?.price || publicData.deliveryOption);
    case AVAILABILITY:
...
class EditListingWizard extends Component {
  constructor(props) {
    ...
    this.showPayoutDetails = this.showPayoutDetails.bind(this);
  }
  ...
  handlePublishListing(id) {
    const { onPublishListingDraft, currentUser, stripeAccount, listing } = this.props;
    const price = listing?.attributes?.price;
    const isPriceZero = price?.amount === 0;
    ...
    if (true) {
      // if ((stripeConnected && !requirementsMissing) || isPriceZero) {
  ...
  render() {
    ...
    function handleShowPayoutDetails() {
      const stripeConnected =
        currentUser && currentUser.stripeAccount && !!currentUser.stripeAccount.id;
      const stripeAccountData = stripeConnected ? getStripeAccountData(stripeAccount) : null;
      const requirementsMissing =
    ...
    const category = currentListing?.attributes?.publicData?.category || params.category;
    const subcategory = currentListing?.attributes.publicData.subCategory || params.subcategory;
    if (!category) {
      return <NamedRedirect name="LandingPage" />;
    }
    let listingCheck = config.custom.listingTypes.find(item => item.key === category);
    if (!listingCheck) {
      return <NamedRedirect name="LandingPage" />;
    }
    let subCategoryCheck = config.custom.categoryOptions.find(item => item.key === subcategory);
    // if (!subCategoryCheck && category === 'service') {
    //   return <NamedRedirect name="LandingPage" />;
    // }

    if (category === 'service' || category === 'event') {
      (TABS = [DESCRIPTION]),
        (TABS = [...TABS].includes(LOCATION) ? [...TABS] : [...TABS, LOCATION]);
      TABS = [...TABS].includes(PRICING) ? [...TABS] : [...TABS, PRICING];
      // TABS = [...TABS].includes(AVAILABILITY) ? [...TABS] : [...TABS, AVAILABILITY];
      TABS = [...TABS].includes(PHOTOS) ? [...TABS] : [...TABS, PHOTOS];
    } else {
      (TABS = [DESCRIPTION]),
        (TABS = [...TABS].includes(LOCATION) ? [...TABS] : [...TABS, LOCATION]);
      TABS = [...TABS].includes(PRICING) ? [...TABS] : [...TABS, PRICING];
          ...
          {TABS.map(tab => {
            return (
              <EditListingWizardTab
                ...
                showPayoutDetails={this.showPayoutDetails}
              />
            );
          })}
        </Tabs>
