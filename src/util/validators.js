...
import { isValidPhoneNumber, isPossiblePhoneNumber, ParseError } from 'react-phone-number-input';
...
export const validatePhoneNumber = message => value => {
  if (!value) return;
  const validPhoneNumberCheck = isValidPhoneNumber(value);
  const isPossiblePhoneNumberCheck = isPossiblePhoneNumber(value);
  const condition = validPhoneNumberCheck && isPossiblePhoneNumberCheck;
  return condition ? VALID : message;
};
export const urlValidate = text => {
  const urlRegex = /(?:https?:\/\/)?[-a-zA-Z0-9@:%._\+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b([-a-zA-Z0-9()@:%_\+.~#?&//=]*)?/gi;

  // const urlRegex = /[-a-zA-Z0-9@:%._\+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b([-a-zA-Z0-9()@:%_\+.~#?&//=]*)?/gi;
  // return html  with url clickable
  return text.replace(urlRegex, url => {
    return `<a style="color: blue" href="${
      url.match(/^https?:\/\//i) ? url : `http://${url}`
    }" target="_blank">${url}</a>`;
  });
};
export const minSuneValue = (message, minValue) => value => {
  return value >= minValue ? VALID : message;
};
export const quantityValidation = (message, quantity) => value => {
  const valueAsNumber = +value;

  return quantity >= valueAsNumber ? VALID : message;
};
